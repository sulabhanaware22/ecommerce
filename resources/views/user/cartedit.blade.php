<x-home-master>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="home_screen">
                        ...
                        <div class="container">
                            <br><br>
                            <form id="edit_cartdetails_form" action="{{route('edit-cart')}}" method="post">
                                <div class="row">
                                    <div class="col-md-8 offset-4 col-xs-12 col-sm-8 offset-4">
                                        <h3>
                                            @if(isset($product->p_name) && $product->p_name != '')
                                            {{$product->p_name}}
                                            @else
                                            ----
                                            @endif
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <img src="{{config('constants.IMAGE_PATH').$product->product_image}}" width="300px" height="300px">
                                    </div>
                                    <div class="col-md-8">
                                        <h3>
                                ...
                                <div class="row">
                                    <div class="col-md-3 offset-4">
                                        <select id="product_quantity" class="form-control" name="product_quantity">
                                            ...
                                            <option value="{{$value['id']}}" @if(isset($product->ref_id) && $product->ref_id != '')
                                                @if($value['id'] == $product->p_qty)
                                                selected="selected"
                                                @endif
                                                @endif
                                                >{{$value['name']}}</option>
                                ...
                                <div class="row">
                                    <div class="col-md-3 offset-4">
                                        <input type="hidden" name="cart_id" value="{{$product->ref_id}}" />
                                        <input type="submit" value="EDIT CART" id="edit_to_cart"></input>
                                    </div>
                                </div>
                            </form>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#edit_cartdetails_form').on('submit', function(event) {
                event.preventDefault();
                data = $("#edit_cartdetails_form").serialize();


                // alert("the value is" + CKEDITOR.instances.productDescription.getData());
                $.ajax({
                    url: "{{ config('constants.BASE_PATH') }}user/editcartitem/",
                    type: 'post',
                    data: data,
